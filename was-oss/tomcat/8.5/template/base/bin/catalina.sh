...
#                   as its stdin the stdout and stderr from the LENA java
...
#                   only be used by LENA itself, not by the stop process,
...
#                   should be used by LENA and also by the stop process,
#                   the version command etc.
...
#                   used
#
#   CATALINA_LOGGING_CONFIG (Optional) Override LENA's logging config file
#                   Example (all one line)
...
#
#   LOGGING_MANAGER (Optional) Override LENA's logging manager
#                   Example (all one line)
#                   LOGGING_MANAGER="-Djava.util.logging.manager=org.apache.juli.ClassLoaderLogManager"
#
#   UMASK           (Optional) Override LENA's default UMASK of 0027
#
#   USE_NOHUP       (Optional) If set to the string true the start command will
#                   use nohup so that the LENA process will ignore any hangup
...
# Add the JAVA 9 specific start-up parameters required by LENA
...
if [ "$1" != "version" ] ; then
	...
	if [ $have_tty -eq 1 ]; then
	  echo "Using LENA_HOME :     $LENA_HOME"
	  #echo "Using CATALINA_BASE:   $CATALINA_BASE"
	  #echo "Using CATALINA_HOME:   $CATALINA_HOME"
	  ...
	  if [ ! -z "$CATALINA_PID" ]; then
	    echo "Using SERVER_PID:    $CATALINA_PID"
	  fi
	  echo "Using SERVER_HOME :   $INSTALL_PATH"
	  echo "Using SERVER_ID :     $SERVER_ID"
	  echo "Using INSTANCE_NAME : $INST_NAME"
	fi
fi	
...
if [ "$1" = "debug" ] ; then
    ...
    if [ -f "$CATALINA_PID" ]; then
      if [ -s "$CATALINA_PID" ]; then
        ...
        if [ -r "$CATALINA_PID" ]; then
          ...
          if [ $? -eq 0 ] ; then
            echo "LENA appears to still be running with PID $PID. Start aborted."
            echo "If the following process is not a LENA process, remove the PID file and try again:"
            ps -f -p $PID
            exit 1
          else
  ...
  if [ ! -z "$CATALINA_PID" ]; then
  	for loop_idx in 1 2 3 4 5
  	do
	  	if [ "`uname -s`" = "HP-UX" ]; then
		  	TEMP_PID_NUM=`ps -efx | grep java | grep ${CATALINA_BASE} | grep was_cname=${INST_NAME} | awk {'print $2'}`
	  	else
		  	TEMP_PID_NUM=`ps -ef | grep java | grep ${CATALINA_BASE} | grep was_cname=${INST_NAME} | awk {'print $2'}`
	  	fi
	  	
	  	if [ ! -z "$TEMP_PID_NUM" ]; then
	  		echo "$TEMP_PID_NUM" > "$CATALINA_PID"
	  		break;
	  	fi
	  	
	  	echo "Waiting for LENA startup."
	  	sleep 1
	  	
	    #echo $! > "$CATALINA_PID"
    done
  	if [ -z "$TEMP_PID_NUM" ]; then
  		echo "LENA startup failed."
  		exit 1;
  	fi
  fi
  ...
  if [ ! -z "$CATALINA_PID" ]; then
    if [ -f "$CATALINA_PID" ]; then
      ...
      echo "\$CATALINA_PID was set but the specified file does not exist. Is LENA running? Stop aborted."
      exit 1
    fi
  fi
      ...
      while [ $SLEEP -ge 0 ]; do
        ...
        if [ $? -gt 0 ]; then
          ...
          if [ $? != 0 ]; then
            if [ -w "$CATALINA_PID" ]; then
              cat /dev/null > "$CATALINA_PID"
              # If LENA has stopped don't try and force a stop with an empty PID file
              FORCE=0
            else
        ...
        if [ $SLEEP -eq 0 ]; then
          echo "LENA did not stop in time."
          if [ $FORCE -eq 0 ]; then
  ...
  if [ $FORCE -eq 1 ]; then
    if [ -z "$CATALINA_PID" ]; then
      ...
      if [ -f "$CATALINA_PID" ]; then
        PID=`cat "$CATALINA_PID"`
        echo "Killing LENA with the PID: $PID"
        kill -9 $PID
        while [ $KILL_SLEEP_INTERVAL -ge 0 ]; do
            ...
            if [ $? -gt 0 ]; then
                ...
                echo "The LENA process has been killed."
                break
            fi
        ...
        if [ $KILL_SLEEP_INTERVAL -lt 0 ]; then
            echo "LENA has not been killed completely yet. The process might be waiting on some system call or might be UNINTERRUPTIBLE."
        fi
      fi
    fi
  fi
  ...
  echo "  version           What version of LENA are you running?"
