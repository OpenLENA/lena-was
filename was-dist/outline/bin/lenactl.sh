...
check_javahome() {
  ...
  while true; do
    ...
    if is_valid_javahome ${input_java_path}; then
      ...
      echo "${input_java_path}" >> ${LENA_HOME}/etc/info/java-home.info
      return
    else
      echo "JAVA_HOME is invalid. Please check if jdk is installed."
    fi
...
INSTALLER_LIB_PATH=${LENA_HOME}/modules/lena-ctl/lib
...
_JAVA_OPTS="-Duser_java.home=${JAVA_HOME} -Dlena.home=${LENA_HOME} -Dhostname=${HOSTNAME} -Drun_user=${RUN_USER} -Dis_debug_enabled=${IS_DEBUG_ENABLED} -Dlog.home=${LOG_HOME} -Dresult.format=${RESULT_FORMAT}"
...
modify)
  ...
  ;;
*)
  debug ${JAVA_HOME}/bin/java ${_CLASSPATH} ${_JAVA_OPTS} "io.openlena.ctl.Main" ${ARGUMENTS}
  check_process
  ${JAVA_HOME}/bin/java ${_CLASSPATH} ${_JAVA_OPTS} "io.openlena.ctl.Main" ${ARGUMENTS}
  ;;
esac
